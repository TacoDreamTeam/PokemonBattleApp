...
const API_URL = 'http://localhost:3000/'; //insert our server
const HTTP_OPTIONS = {
...
export class AuthorizeService {
   ...
   login(loginData: any): Observable<any> {
    ...
    return this.http.post<any>(API_URL + 'oauth/token', body, HTTP_OPTIONS)
      .pipe(
        ...
        catchError(AuthorizeService.handleError)
      );
  }
  ...
  register(data: any): Observable<any> {
    return this.http.post<any>(API_URL + 'oauth/signup', data)
      .pipe(
        tap(_ => AuthorizeService.log('register')),
        catchError(AuthorizeService.handleError)
      );
  }
  ...
  secured(): Observable<any> {
    return this.http.get<any>(API_URL + 'secret')
      .pipe(catchError(AuthorizeService.handleError));
  }
}
